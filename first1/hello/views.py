...
def task1(request):
    book = Book.objects.filter(is_available=True).order_by('title')
    return render(request, 'task1.html', {'books': book})

def task2(request):
    id_a =1
    book = Book.objects.filter(author_id=id_a).order_by('-publication_year')
    return render(request, 'task2.html', {'books': book})
...
def task4(request):
    ...
    return render(request, 'task4.html', {'books': book})
